#include <time.h>
#include "sst.h"
static int _nextid = 1;/*added for better sorting*/
...
void freeze(int boss) {
	...
	fwrite(kid, sizeof(kid), 1, fp);
	fwrite(d.cid, sizeof(d.cid), 1, fp);
...
void thaw(void) {
	...
	fread(kid, sizeof(kid), 1, fp);
	fread(d.cid, sizeof(d.cid), 1, fp);
...
void abandn(void) {
	...
	imine=0;
	nprobes=0;/*added this. Ye FQ has no probes right?*/
	iscraft=-1; /* Gallileo disappears */ /*fixed this apparent bug*/
	/* Resupply ship */
	condit=IHDOCKED;
...
void setup(void) {
	...
	for (i = 1; i <= incom; i++) {
		...
		d.cx[i] = ix;
		d.cy[i] = iy;
		d.cid[i] = i;/*added id for better sorting*/
	}
	...
	if (Rand() < 0.1) {/*changed to allow thing in tournament game*/
		iran8(&thingx, &thingy);
	}
	else {
		thingx = thingy = 0;
...
int choose(void) {
	...
	while (TRUE) {
		...
		if (isit("tournament")) {
			...
			tourn = (int)aaitem;
			thingx = -1;
			SetRandomSeed((int)aaitem);
			Rand(); Rand(); Rand(); Rand();/*added these to be insync with net version*/
			break;
		}
		if (isit("frozen")) {
			thaw();
			...
			/*added logic here to set tournament seed if set*/
			if(tourn)
			{
				//srand(tourn);
			}
...
void newqad(int shutup) {
	...
	if (quadnum >= 100) {
		...
		for (i = 1; i <= klhere; i++) {
			...
			kid[i] = _nextid++;/*added id for better sorting*/
		}
		...
		if (i <= d.remcom) {
			...
			kid[klhere] = _nextid++;/*added id for better sorting*/
			comhere = 1;
		}
		...
		if (quadx == d.isx && quady == d.isy) {
			...
			kid[1] = _nextid++;/*added id for better sorting*/
			iscate = 1;
			ishere = 1;
		}
	}
...
void sortkl(void) {
	...
	do {
		sw = FALSE;
		for (j = 1; j < nenhere; j++)
			if( (kdist[j] > kdist[j+1]) || ( kdist[j] == kdist[j+1] && kid[j] > kid[j+1] )) {/*changed sorting test to take into account id*/
				sw = TRUE;
				t = kdist[j];
				kdist[j] = kdist[j+1];
				kdist[j+1] = t;
				...
				kpower[j] = kpower[j+1];
				kpower[j+1] = t;
				k = kid[j];/*added sorting of id field*/
				kid[j] = kid[j+1];
				kid[j+1] = k;
			}
	} while (sw);
