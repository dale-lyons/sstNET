...
void ram(int ibumpd, int ienm, int ix, int iy) {
	...
	for (l=1; l <= ndevice; l++) {
		...
		if(l == DSHUTTL && iscraft != 1) continue;/*added this exclusion, cannot damage shuttle is not on board*/
...
void torpedo(double course, double r, int inx, int iny, double *hit) {
			...
			case IHB: /* Hit a base */
				...
				for (ll=1; ll<=d.rembase; ll++) {
					if (d.baseqx[ll]==quadx && d.baseqy[ll]==quady) {
						...
						d.baseid[ll]=d.baseid[d.rembase];/*added for better sorting*/
						break;
					}
				}
				quad[ix][iy]=IHDOT;
	...
	if (shoved) {
		...
		/*sortkl();*/ /*Cannot resort here. may be called while enemies attacking*/
		return;
	}
	skip(1);
...
void attack(int k) {
	...
	int percent, ihurt=0, l, i=0, jx, jy, iquad, itflag, num;
	...
	for (l=1; l <= nenhere; l++) {
		...
		if (itflag) {
			...
			if(num != nenhere) l--;/*fix bug where Romulan torp kills a local baddie*/
		...
		/*%g seems to be different in c#, use this logic instead*/
		proutn("Hit ");
		cramf(hit, 0, 1);
		proutn(" energy ");
		cramf(energy, 0, 1);
	...
	sortkl();/*ok to sort now, all enemies have attacked*/
	if (energy <= 0) {
...
void deadkl(int ix, int iy, int type, int ixx, int iyy) {
	...
	if (type == IHR) {
		...
		switch (type) {
			case IHC:
				...
				d.cid[i] = d.cid[d.remcom];/*added this for sorting*/
				d.cx[d.remcom] = 0;
				d.cy[d.remcom] = 0;
				d.cid[d.remcom] = 0;/*added this for sorting*/
				d.remcom--;
				sortcom();
				future[FTBEAM] = 1e30;
				if (d.remcom != 0)
				...
				/*fixed this bug*/
				/* if we kill the commander that is currently attacking a base, stop the attack.*/
				if(quadx==batx && quady==baty)
					future[FCDBAS] = 1e30;
	...
	if (i <= nenhere)  {
		for (j=i; j<=nenhere; j++) {
			...
			kid[j] = kid[j+1];/*added id for sorting*/
		}
	}
	kx[nenhere+1] = 0;
	ky[nenhere+1] = 0;
	...
	kavgd[nenhere+1] = 0;
	kpower[nenhere+1] = 0;
	kid[nenhere+1] = 0;/*added id for sorting*/
	return;
}
...
void photon(void) {
	...
	for (;;) {
		if (key == IHALPHA) {
			...
			if (n > 3) {
				...
				key = IHEOL;
				continue;/*fixed bug where more than 3 torps could be fired*/
			}
			if (n <= torps) break;
			chew();
			key = IHEOL;
	...
	for (i = 1; i <= n && !osuabor; i++) {
		...
		sortkl();/*sorting was in torpedo, removed so do it now*/
	}
...
static int checkshctrl(double rpow) {
	...
	if (icas) {
		...
		casual += icas;/*should be adding casualties*/
	}
	skip(1);
