...
static int tryexit(int lookx, int looky, int ienm, int loccom, int irun) {
	...
	if (damage[DSRSENS] == 0.0 || damage[DLRSENS] == 0.0 ||
		...
		proutn("***");
		cramen(ienm);
		proutn("(");/*next 3 lines added for better reporting*/
		cramlc(0, kx[loccom], ky[loccom]);
		proutn(")");
		proutn(" escapes to");
		cramlc(1, iqx, iqy);
	...
	else if (ienm==IHC) {/* added extra test to fix bug*/
		for (l=1; l<=d.remcom; l++) {
...
void scom(int *ipage) {
	...
	if (iscate==0 && flag) {
		...
		if (d.rembase > 1) {
			...
			do {
				iswitch = 0;
				for (i=1; i <= d.rembase-1; i++) {/*bug, not processing enough bases*/
	...
	iqx = d.isx + ideltax;
	iqy = d.isy + ideltay;/*fixed this bug???*/
