...
void events(void) {
	...
	double fintim = d.date + Time, datemin, xtime, repair, yank, r;/*added r for random sync below*/
		...
		switch (line) {
			...
			case FTBEAM: /* Tractor beam */
				if (line==FTBEAM) {
					...
					sortcom();/*added this for better sorting*/
					i = Rand()*d.remcom+1.0;
				...
				/* Added assignment to r to keep random number in sync with pre-bug fix*/
				if (iscraft==0) r = Rand();
				if (iscraft==0 && inorbit) {
					skip(1);
					if (r >0.5) {
						...
						d.plnets[iplnet].known=2;/*fix bug, shuttle left on planet surface*/
					}
				}
				if (line==0) {
					quadx = d.isx;
			...
			case FCDBAS: /* Commander succeeds in destroying base */
				...
				for (i=1; i <= d.rembase; i++)
					if (d.baseqx[i]==batx && d.baseqy[i]==baty) {
						...
						d.baseid[i]=d.baseid[d.rembase];/*added for base sorting*/
					}
				d.rembase--;
				sortbase();
				if (isatb == 2) {
			...
			case FDSPROB: /* Move deep space probe */
				...
				if(proben <= 0)/*added this logic, no probes no future events*/
				{
					future[FDSPROB] = 1e30;
				}
				break;
...
void snova(int insx, int insy) {
	...
	if (insy== 0) {
		...
		if (nqx != quadx || nqy != quady || justin != 0) {/*fix bug*/
	...
	if (nqx==d.isx && nqy == d.isy) {
		...
		/* if we kill the supercommander that is currently attacking a base, stop the attack.*/
		future[FSCDBAS] = 1e30;
	...
	if (d.remcom) {
		...
		for (l = 1; l <= maxloop; l++) {
			if (d.cx[l] == nqx && d.cy[l] == nqy) {
				...
				d.cid[l] = d.cid[d.remcom];/*added for better sorting*/
	...
	if (d.rembase) {
		...
		for (l = 1; l <= maxloop; l++)
			if (d.baseqx[l]==nqx && d.baseqy[l]==nqy) {
				...
				d.baseid[l] = d.baseid[d.rembase];/*added for better sorting*/
