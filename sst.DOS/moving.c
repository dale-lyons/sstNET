			...
			if (iquad != IHDOT) {
				...
				switch (iquad) {
					...
					case IHR:
						...
						finalx = sectx;
						finaly = secty;
						ix=sectx; iy=secty;//fix bug. Need to compute distance from ship, not the block object!
						break;
					case IHBLANK:
						skip(1);
...
void warp(int i) {
	...
	/*added this logic to get around a bug. If tractor beamed at this point
	the ship was moved but not set into its new location.*/
	if (d.date+Time >= future[FTBEAM])
	{
		events();
		return;
...
void timwrp() {
	...
	if (d.snap && Rand() < 0.5) {
		...
		future[FSNAP] = d.date +expran(0.25*d.remtime); /* next snapshot will
													   be sooner */
		if (d.nscrem) future[FSCMOVE] = d.date + 0.2777;/*fixed this bug*/
		isatb = 0;
...
void help(void) {
	...
	for (l = 1; l <= 5; l++) {
		...
		if (ix>=1 && ix<=10 && iy>=1 && iy<=10 && quad[ix][iy]==IHDOT) {
			...
			/*I added this code cause it makes sense.Should we not update the enemy table at this points?*/
			for (l=1; l<=nenhere; l++)
				kdist[l] = kavgd[l] = sqrt(square(sectx-kx[l])+square(secty-ky[l]));
			sortkl();
			return;
		}
	}
