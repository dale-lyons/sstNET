...
void preport(void) {
	...
	skip(1);
	for (i = 1; i <= inplan; i++) {
		if (d.plnets[i].x !=0 && (d.plnets[i].known || isdebug())
#ifdef DEBUG
			...
			if (d.plnets[i].known==2 && iscraft != -1) /*added this extra check in case shuttle is out of action*/
...
void sensor(void) {
	...
	prout(".");
	if (d.plnets[iplnet].known==2 && iscraft != -1) /*added this extra check in case shuttle is out of action*/
